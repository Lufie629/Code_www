...
def calculate(cnt):
    ...
    for i in range(5):
        ...
        end_time = time.time()
        y_predict_pro = a.predict_proba(X[test_mask])[:, 0]
...
# # dict[split['id'][i]]中存放的是每一个id对应的序号，按照train,val,test顺序排列
...
#所以我这里需要用到的只是cat和num 6,11

path='../../Datasets_process/Data/detptData/'
cat_features = torch.load(path + "det_new_cat_properties_tensor.pt", map_location="cpu")
prop_features = torch.load(path + "det_new_num_properties_tensor.pt", map_location="cpu")
X = torch.cat((cat_features[:,:8],prop_features[:,1:2]), dim=1)
print(prop_features.shape)
# X = cat_features[:,:8]
y = torch.load(path + "det_new_label.pt", map_location="cpu")
datalen=y.shape[0]
print("loading index...")
train_mask = torch.arange(0, int(datalen*0.7))
val_mask = torch.arange(int(datalen*0.7),int(datalen*0.9))
test_mask = torch.arange(int(datalen*0.90), datalen)
