...
class Twibot20(Dataset):
    def __init__(self,root='../../Datasets_process/Data/detptData/',device='cpu',process=True,save=True):
        self.root = root
        self.device = device
        
    def load_labels(self):
        print('Loading labels...',end='   ')
        # path=self.root+'/data/gluo/Bots_code/RGCN_prediction/Data/detptData/label.pt'
        path='../datasets/detptData/label.pt'
    ...
    def Des_Preprocess(self):
        ...
        path='../datasets/detptData/description.npy'
        if not os.path.exists(path):
            description=[]
    ...
    def Des_embbeding(self):
        ...
        path = '../datasets/detptData/det_des_tensor.pt'
        if not os.path.exists(path):
    ...
    def dataloader(self):
        ...
        category_prop = torch.load(path + "det_new_cat_properties_tensor.pt", map_location="cpu").to(self.device)
        num_prop = torch.load(path + "det_new_num_properties_tensor.pt", map_location="cpu").to(self.device)
        tweet_features = torch.load(path + "det_new_tweets_tensor.pt", map_location="cpu").to(self.device)
        des_tensor = torch.load(path + "det_new_des_tensor.pt", map_location="cpu").to(self.device)
        tweets_tensor=tweet_features.mean(dim=1)
        
        # tweets_tensor = tweet_features.view(14062, 100*768)
        
        edge_index = torch.load(path + "det_new_edge_index.pt", map_location="cpu").to(self.device)
        edge_type = torch.load(path + "det_new_edge_type.pt", map_location="cpu").to(self.device)
        labels = torch.load(path + "det_new_label.pt", map_location="cpu").to(self.device)
        datalen=labels.shape[0]
        train_idx = torch.arange(0, int(datalen*0.7))
        val_idx = torch.arange(int(datalen*0.7),int(datalen*0.9))
        test_idx = torch.arange(int(datalen*0.9), datalen)
        print(labels)
        # print(cat_features.shape, prop_features.shape, tweet_features.shape, des_features.shape)
