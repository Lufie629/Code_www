...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.linear_layer = nn.Linear(101, 51)
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs,tweet_embs):
        ...
        # dec_outputs=torch.cat((dec_outputs,tweet_embs),2)
        # print("tweet_embs shape", tweet_embs.shape)

        # # tweet 变换维度
        # # 将tweet_embs的维度变换成(batch_size, features)
        # tweet_embs = tweet_embs.view(-1, 101)
        # # 通过线性层进行变换
        # output = self.linear_layer(tweet_embs)
        # # 将output的维度变回原来的形状
        # output = output.view(-1, 51, 30)
